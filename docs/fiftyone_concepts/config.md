...
| `default_batch_size` | `FIFTYONE_DEFAULT_BATCH_SIZE` | `None` | A default batch size to use when [applying models to datasets](../data_and_models/model_zoo/api.html#model-zoo-apply). |
...
| `model_zoo_dir` | `FIFTYONE_MODEL_ZOO_DIR` | `~/fiftyone/__models__` | The default directory in which to store models that are downloaded from the<br>[FiftyOne Model Zoo](../data_and_models/model_zoo/index.html#model-zoo). |
| `model_zoo_manifest_paths` | `FIFTYONE_MODEL_ZOO_MANIFEST_PATHS` | `None` | A list of manifest JSON files specifying additional zoo models. See<br>[adding models to the zoo](../data_and_models/model_zoo/api.html#model-zoo-add) for more information. |
